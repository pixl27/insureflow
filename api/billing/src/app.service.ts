...
export class AppService implements OnModuleInit {
  ...
  onModuleInit() {
    eventBus.subscribe('ContractCreated', async (event: any) => {
  ...
  async calculateDynamicCommission(contractId: string, baseAmount: number) {
  ...
  async remindInvoice(invoiceId: string, data: { channel?: 'EMAIL' | 'SMS' | 'PORTAL'; note?: string }) {
    const invoice = await this.prisma.invoice.findUnique({
      where: { id: invoiceId },
      include: {
        customer: { include: { contacts: true } }
